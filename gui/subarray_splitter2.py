        ...
        def _load_worker(q):
            ...
            for i, sac in enumerate(sac_files):
                ...
                if captured_dt is None:
                    captured_dt = dt
                elif abs(dt - captured_dt) > 1e-9:
                    q.put(("text", f"Warning: {fname} dt={dt} differs from first dt={captured_dt}"))
            ...
            # Frequency axis (using dt from SAC header)
            nd = ncfs_arr.shape[1]
            actual_dt = captured_dt
